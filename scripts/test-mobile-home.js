...
assert(home.includes('<ProgressRing value={pct} size={72} splitLabel/>'), 'El indicador diario debe ser compacto, separar su etiqueta y usar progreso real');
...
assert(!capture.includes('m2-capture-mic') && !capture.includes('Dictar captura rápida'), 'La barra de captura no debe incluir un botón de audio lateral');
...
assert(home.includes('todaySpend') && home.includes('monthSpend'), 'Finanzas rápidas debe derivar gasto y presupuesto reales');
assert(!home.includes('Categoría principal') && !home.includes('m2-category-summary'), 'Finanzas rápidas no debe mostrar categoría principal');
...
assert(!home.includes('Logros recientes') && !home.includes('m2-achievement'), 'Inicio Mobile/Tablet no debe mostrar Logros recientes');
assert(home.includes("if(id==='pomodoro')navigate('pomodoro')"), 'El acceso Pomodoro debe abrir la sección Pomodoro');
assert(home.includes("wallet:'add',type:'expense'"), 'Nuevo gasto debe abrir Wallet Agregar en modo Gasto');
...
assert(css.includes('grid-auto-flow:column') && css.includes('grid-auto-columns:minmax(70px,1fr)'), 'Los accesos rápidos deben usar una sola fila compacta en teléfono');
...
assert(app.includes('voice-field is-wide voice-field-date') && app.includes('voice-field is-wide voice-field-time') && app.includes('-webkit-appearance: none'), 'Fecha y hora deben igualar el ancho de Título y neutralizar el ancho intrínseco de Safari');

console.log('Inicio Mobile/Tablet profesional: 39 comprobaciones correctas.');
