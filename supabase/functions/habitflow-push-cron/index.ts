...
const dueDebtNotificationsForUser = (row: any, now: Date) => {
  ...
  (finance.accounts || []).forEach((account: any) => {
    if (!account?.debtDueDate || account.debtReminderEnabled === false) return;

    const tagGroup = tags.find((tag: any) => tag.id === account.tagId)?.group || account.type;
    ...
    const movement = transactions
      .filter((transaction: any) => (transaction.accountId || "") === account.id)
