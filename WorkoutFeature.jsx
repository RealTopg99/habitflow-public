(function registerHabitFlowWorkoutFeature(global) {
  ...
  const ensureWorkoutStyles = () => {
    ...
    style.textContent = `
      ...
      .wr-builder-layout{display:grid;grid-template-columns:minmax(0,1fr) 370px;gap:18px}.wr-builder-main,.wr-builder-side{padding:20px}.wr-builder-name{font-size:19px;font-weight:800}.wr-builder-list{display:grid;gap:8px;margin-top:14px}.wr-builder-row{display:grid;grid-template-columns:32px minmax(180px,1.2fr) 92px 104px 94px minmax(130px,.8fr) 42px;gap:10px;align-items:center;padding:11px;border:1px solid var(--hf-card-border);border-radius:13px;background:var(--hf-input-bg)}.wr-builder-row.is-dragging{opacity:.45}.wr-drag{border:0;background:none;color:var(--hf-muted);padding:5px}.wr-builder-ex{display:grid;grid-template-columns:38px 1fr;gap:9px;align-items:center;min-width:0}.wr-builder-ex img{width:38px;height:38px;border-radius:8px;object-fit:contain;background:var(--hf-surface-soft)}.wr-builder-ex strong{display:block;font-size:12px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.wr-builder-ex span{font-size:9px;color:var(--hf-muted)}.wr-small-input{min-height:40px;padding:0 9px}.wr-row-menu{display:flex;gap:2px}.wr-side-metrics{display:grid;gap:15px;margin:18px 0;padding-bottom:18px;border-bottom:1px solid var(--hf-card-border)}.wr-side-metric span{display:block;font-size:11px;color:var(--hf-muted);margin-bottom:4px}.wr-side-metric strong{font-size:18px}.wr-days{display:grid;grid-template-columns:repeat(7,1fr);gap:6px;margin:10px 0 18px}.wr-day-toggle{height:40px;border-radius:9px;border:1px solid var(--hf-card-border);background:var(--hf-input-bg);color:var(--hf-muted)}.wr-day-toggle.is-active{background:var(--wr-red);color:#fff;border-color:var(--wr-red)}.wr-builder-summary{display:grid;grid-template-columns:1fr 1fr;gap:10px;padding:13px;border:1px solid var(--hf-card-border);border-radius:12px;margin-bottom:14px}.wr-builder-summary span{display:block;font-size:9px;color:var(--hf-muted)}.wr-builder-summary strong{font-size:12px}.wr-validation{color:var(--wr-red);font-size:11px;margin-top:10px}
      .wr-overlay{position:fixed;inset:0;z-index:2600;background:var(--hf-overlay);backdrop-filter:blur(8px);display:grid;place-items:center;padding:20px}.wr-modal{width:min(760px,100%);max-height:min(85vh,820px);overflow:auto;padding:20px}.wr-picker-list{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:12px}.wr-picker-item{display:grid;grid-template-columns:42px minmax(0,1fr) auto;gap:10px;align-items:center;padding:9px;border:1px solid var(--hf-card-border);border-radius:11px;background:var(--hf-input-bg);text-align:left;color:var(--hf-text)}.wr-picker-item img{width:42px;height:42px;object-fit:contain;border-radius:8px;background:var(--hf-surface-soft)}.wr-picker-item span{font-size:10px;color:var(--hf-muted)}
      .wr-session{max-width:none}.wr-session-bar{display:grid;grid-template-columns:220px 1fr auto;gap:22px;align-items:center;padding:16px 20px;margin-bottom:18px}.wr-session-time{display:grid;grid-template-columns:38px 1fr;gap:10px;align-items:center}.wr-session-time svg{color:var(--wr-red)}.wr-session-time strong{font-size:25px;display:block}.wr-session-layout{display:grid;grid-template-columns:minmax(0,1fr) 360px;gap:18px}.wr-current{padding:22px}.wr-current-grid{display:grid;grid-template-columns:minmax(260px,.92fr) minmax(260px,1.08fr);gap:22px;align-items:center;margin:16px 0}.wr-current-media{min-height:300px;display:grid;place-items:center;background:var(--hf-surface-soft);border:1px solid var(--hf-card-border);border-radius:15px;position:relative}.wr-current-media img{width:180px;height:180px;object-fit:contain}.wr-current-stats strong{font-size:26px}.wr-current-weight{font-size:26px;color:var(--wr-red);font-weight:800;margin:12px 0 20px}.wr-rest{display:flex;align-items:center;gap:13px;margin:14px 0}.wr-rest-ring{width:58px;height:58px;border-radius:50%;display:grid;place-items:center;background:conic-gradient(var(--wr-red) var(--rest-progress,0%),var(--hf-track) 0);position:relative}.wr-rest-ring:after{content:'';position:absolute;inset:6px;border-radius:50%;background:var(--hf-surface-strong)}.wr-rest-ring svg{position:relative;z-index:1;color:var(--wr-red)}.wr-set-list{display:grid;gap:6px}.wr-set-row{display:grid;grid-template-columns:52px 1fr 1fr auto;gap:12px;align-items:center;min-height:48px;padding:7px 13px;border:1px solid var(--hf-card-border);border-radius:11px;background:var(--hf-input-bg)}.wr-set-row.is-current{border-left:4px solid var(--wr-red)}.wr-set-row.is-complete{color:var(--wr-success)}.wr-set-row input{width:100%;min-height:34px}.wr-session-side{padding:20px;display:flex;flex-direction:column}.wr-next{display:grid;grid-template-columns:1fr 90px;gap:10px;align-items:center;padding-bottom:18px;border-bottom:1px solid var(--hf-card-border)}.wr-next img{width:90px;height:90px;object-fit:contain}.wr-session-note{min-height:180px;margin:12px 0 16px}.wr-session-side .wr-btn:last-child{margin-top:auto}.wr-paused{padding:9px 12px;border-radius:9px;background:color-mix(in srgb,var(--wr-amber) 12%,transparent);color:var(--wr-amber);font-size:12px;font-weight:800}
  ...
  const SummaryView = ({ workoutData, meta, catalogById, onNavigate, onEditRoutine, onStartRoutine }) => {
    ...
    return <>
      ...
      <section className="wr-card wr-section">
        ...
        {routines.length ? <div className="wr-routine-grid">{routines.slice(0, 3).map((routine, index) => {
          ...
          return <button className="wr-routine-card" key={routine.id} onClick={() => onStartRoutine(routine)} style={{ textAlign: 'left', color: 'inherit' }}>
            <div className="wr-routine-top"><span className="wr-routine-icon">{index % 3 === 0 ? <Dumbbell size={19} /> : index % 3 === 1 ? <Zap size={19} /> : <Activity size={19} />}</span><div className="wr-routine-main"><strong>{routine.name}</strong><span className="wr-meta">{(routine.muscleGroups || []).join(', ') || 'Rutina personalizada'}</span></div><ChevronRight size={16} /></div>
      ...
      <div className="wr-bottom-grid">
        ...
        <section className="wr-card wr-section">
          <div className="wr-section-head"><h2 className="wr-card-title">Base de ejercicios integrada</h2><span className="wr-pill" style={{ margin: 0, color: 'var(--wr-red)' }}>Dataset</span></div>
          <p className="wr-meta" style={{ lineHeight: 1.6 }}>Explora movimientos, técnica y medios cargados bajo demanda.</p>
          <button className="wr-input" style={{ textAlign: 'left', color: 'var(--hf-muted)' }} onClick={() => onNavigate('library')}><Search size={14} style={{ marginRight: 8, verticalAlign: 'middle' }} />Buscar ejercicio...</button>
          <div className="wr-dataset-stats"><div className="wr-dataset-stat"><strong>{meta?.count ?? '—'}</strong><span>Ejercicios</span></div><div className="wr-dataset-stat"><strong>{meta?.categories?.length ?? '—'}</strong><span>Categorías</span></div><div className="wr-dataset-stat"><strong>© Gym visual</strong><span>Medios atribuidos</span></div></div>
          <button className="wr-btn" onClick={() => onNavigate('library')}>Explorar base de ejercicios <ChevronRight size={15} /></button>
        </section>
      </div>
    </>;
  };
  ...
  const LibraryView = ({ catalog, catalogLoading, catalogError, workoutData, onToggleFavorite, onOpenDetail, onCustomExercise }) => {
    ...
    return <>
      ...
      <div className="wr-toolbar">
        ...
        <select className="wr-select" value={target} onChange={e => setTarget(e.target.value)} aria-label="Objetivo"><option value="all">Objetivo</option>{options.targets.map(value => <option value={value} key={value}>{displayLabel(value)}</option>)}</select>
      </div>
  ...
  const DetailView = ({ exercise, loading, error, isFavorite, onBack, onToggleFavorite, onAddToRoutine, onStartNow }) => {
    ...
    return <>
      ...
      <div className="wr-detail-grid">
        <section className="wr-card wr-detail-media"><button className="wr-btn wr-media-toggle" onClick={() => setPlaying(value => !value)}>{playing ? <Pause size={16} /> : <Play size={16} />}{playing ? 'Pausar' : 'Ver animación'}</button><img src={dataset.resolveExerciseMedia(playing ? exercise.gif_url : exercise.image)} alt={`Demostración de ${exercise.name}`} decoding="async" /><a className="wr-attribution" href="https://gymvisual.com/" target="_blank" rel="noreferrer">{exercise.attribution || '© Gym visual — https://gymvisual.com/'}</a></section>
        <section className="wr-card wr-detail-info"><h1>{exercise.name}</h1><div className="wr-muted">{displayLabel(exercise.category, CATEGORY_ES)} · {displayLabel(exercise.equipment, EQUIPMENT_ES)}</div>
          <div className="wr-info-grid"><div className="wr-info-box"><span>Objetivo</span><strong>{displayLabel(exercise.target)}</strong></div><div className="wr-info-box"><span>Equipo</span><strong style={{ color: 'var(--wr-blue)' }}>{displayLabel(exercise.equipment, EQUIPMENT_ES)}</strong></div><div className="wr-info-box"><span>Grupo muscular</span><strong style={{ color: 'var(--wr-success)' }}>{displayLabel(exercise.muscle_group)}</strong></div></div>
          <h2 className="wr-card-title">Músculos trabajados</h2><div className="wr-muscles"><span className="wr-muscle">{displayLabel(exercise.target)}</span>{(exercise.secondary_muscles || []).map(value => <span className="wr-muscle" key={value}>{displayLabel(value)}</span>)}</div>
  ...
  const ExercisePicker = ({ catalog, selectedIds, onAdd, onClose }) => {
    const [search, setSearch] = useState('');
    const query = dataset.normalizeSearch(search);
    const items = useMemo(() => catalog.filter(item => !selectedIds.has(String(item.id)) && (!query || dataset.searchableText(item).includes(query))).slice(0, 60), [catalog, selectedIds, query]);
    return <div className="wr-overlay" onMouseDown={event => { if (event.target === event.currentTarget) onClose(); }}><section className="wr-card wr-modal" role="dialog" aria-modal="true" aria-label="Añadir ejercicio"><div className="wr-section-head"><div><h2 className="wr-card-title">Añadir ejercicio</h2><div className="wr-meta">Busca en el catálogo real; se muestran hasta 60 coincidencias.</div></div><button className="wr-btn wr-icon-btn" onClick={onClose} aria-label="Cerrar"><X size={16} /></button></div><label className="wr-search"><Search size={16} /><input className="wr-input" autoFocus value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar por nombre, músculo o equipo" /></label><div className="wr-picker-list">{items.map(item => <button className="wr-picker-item" key={item.id} onClick={() => onAdd(item)}><img src={dataset.resolveExerciseMedia(item.image)} loading="lazy" decoding="async" alt="" /><span style={{ minWidth: 0 }}><strong style={{ display: 'block', color: 'var(--hf-text)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{item.name}</strong><span>{displayLabel(item.target)} · {displayLabel(item.equipment, EQUIPMENT_ES)}</span></span><Plus size={16} /></button>)}</div>{!items.length && <div className="wr-meta" style={{ padding: 18, textAlign: 'center' }}>No hay coincidencias disponibles.</div>}</section></div>;
  };
  ...
  const RoutineBuilder = ({ catalog, routine, prefillExerciseId, onSave, onCancel }) => {
    ...
    const addExercise = (exercise) => {
      setItems(current => [...current, { id: uid('routine_ex'), exerciseId: String(exercise.id), order: current.length, sets: 3, reps: '8–12', restSeconds: 90, notes: '', targetWeight: 0 }]);
      setShowPicker(false);
    };
    const muscleGroups = [...new Set(items.map(item => byId.get(item.exerciseId)?.target || byId.get(item.exerciseId)?.muscle_group).filter(Boolean))];
    const draft = { exercises: items };
    const validate = () => {
      if (!name.trim()) return 'Escribe un nombre para la rutina.';
      if (!items.length) return 'Añade al menos un ejercicio.';
      if (items.some(item => Number(item.sets) <= 0 || !String(item.reps).trim() || Number(item.restSeconds) < 0)) return 'Revisa series, repeticiones y descanso.';
      return '';
    };
    const save = async () => {
      ...
      if (saving) return;
      setSaving(true);
      await onSave({ id: routine?.id || uid('routine'), name: name.trim(), description: description.trim(), type: routine?.type || 'Fuerza', scheduledDays, exercises: items.map((item, order) => ({ ...item, order })), muscleGroups, estimatedDuration: routineDuration(draft), createdAt: routine?.createdAt || nowIso(), updatedAt: nowIso() });
      setSaving(false);
    };
    return <>
      <div className="wr-detail-head"><button className="wr-btn wr-btn-quiet" onClick={onCancel}><ArrowLeft size={16} />Volver</button><div className="wr-inline-actions"><button className="wr-btn" onClick={() => setShowPicker(true)}><Plus size={15} />Añadir ejercicio</button></div></div>
      <div className="wr-builder-layout">
        <section className="wr-card wr-builder-main"><label className="wr-eyebrow" htmlFor="routine-name">Nombre de rutina</label><input id="routine-name" className="wr-input wr-builder-name" value={name} onChange={e => { setName(e.target.value); setError(''); }} placeholder="Ej. Torso · Fuerza" /><textarea className="wr-textarea" style={{ marginTop: 10, minHeight: 74 }} value={description} onChange={e => setDescription(e.target.value)} placeholder="Descripción opcional" />
          <div className="wr-section-head" style={{ marginTop: 18 }}><div><h2 className="wr-card-title">Ejercicios</h2><div className="wr-meta">Arrastra para reordenar o usa los controles de teclado.</div></div><button className="wr-btn" onClick={() => setShowPicker(true)}><Plus size={14} />Añadir ejercicio</button></div>
          {items.length ? <div className="wr-builder-list">{items.map((item, index) => {
            ...
            return <div className={`wr-builder-row ${dragging === item.id ? 'is-dragging' : ''}`} key={item.id} draggable onDragStart={() => setDragging(item.id)} onDragEnd={() => setDragging('')} onDragOver={event => event.preventDefault()} onDrop={() => dropOn(item.id)}>
              <button className="wr-drag" aria-label={`Arrastrar ${exercise?.name || 'ejercicio'}`}><GripVertical size={17} /></button><div className="wr-builder-ex"><img loading="lazy" decoding="async" src={dataset.resolveExerciseMedia(exercise?.image)} alt="" /><div><strong>{exercise?.name || 'Ejercicio no disponible'}</strong><span>{displayLabel(exercise?.target || exercise?.muscle_group)}</span></div></div>
              <label className="wr-field-sets"><input className="wr-input wr-small-input" type="number" min="1" value={item.sets} onChange={e => updateItem(item.id, { sets: Math.max(1, Number(e.target.value || 1)) })} aria-label={`Series de ${exercise?.name}`} /></label>
              <label className="wr-field-reps"><input className="wr-input wr-small-input" value={item.reps} onChange={e => updateItem(item.id, { reps: e.target.value })} aria-label={`Repeticiones de ${exercise?.name}`} /></label>
              <label className="wr-field-rest"><input className="wr-input wr-small-input" type="number" min="0" step="15" value={item.restSeconds} onChange={e => updateItem(item.id, { restSeconds: Math.max(0, Number(e.target.value || 0)) })} aria-label={`Descanso de ${exercise?.name}`} /></label>
              <label className="wr-notes-field"><input className="wr-input wr-small-input" value={item.notes} onChange={e => updateItem(item.id, { notes: e.target.value })} placeholder="Notas" aria-label={`Notas de ${exercise?.name}`} /></label>
              <div className="wr-row-menu"><button className="wr-fav" disabled={index === 0} onClick={() => moveItem(item.id, -1)} aria-label={`Subir ${exercise?.name}`}><ArrowUp size={14} /></button><button className="wr-fav" disabled={index === items.length - 1} onClick={() => moveItem(item.id, 1)} aria-label={`Bajar ${exercise?.name}`}><ArrowDown size={14} /></button><button className="wr-fav wr-btn-danger" onClick={() => removeItem(item.id)} aria-label={`Eliminar ${exercise?.name}`}><Trash2 size={14} /></button></div>
            </div>;
          })}</div> : <EmptyState icon={Dumbbell} title="Rutina vacía" text="Añade ejercicios reales del catálogo para configurar el plan." action={() => setShowPicker(true)} actionLabel="Añadir ejercicio" />}
        </section>
        <aside className="wr-card wr-builder-side"><h2 className="wr-card-title">Resumen de la rutina</h2><div className="wr-side-metrics"><div className="wr-side-metric"><span>Duración estimada</span><strong>{routineDuration(draft)} min</strong></div><div className="wr-side-metric"><span>Series totales</span><strong>{routineSets(draft)}</strong></div><div className="wr-side-metric"><span>Músculos principales</span><strong style={{ fontSize: 14 }}>{muscleGroups.map(displayLabel).join(', ') || '—'}</strong></div></div><h3 className="wr-card-title" style={{ fontSize: 14 }}>Días programados</h3><div className="wr-days">{DAY_OPTIONS.map(([id, label]) => <button key={id} className={`wr-day-toggle ${scheduledDays.includes(id) ? 'is-active' : ''}`} onClick={() => setScheduledDays(current => current.includes(id) ? current.filter(value => value !== id) : [...current, id])} aria-pressed={scheduledDays.includes(id)}>{label}</button>)}</div><div className="wr-builder-summary"><div><Dumbbell size={14} color="var(--wr-red)" /><strong>{items.length}</strong><span>Ejercicios</span></div><div><Layers3 size={14} color="var(--wr-red)" /><strong>{routineSets(draft)}</strong><span>Series</span></div><div><Clock3 size={14} color="var(--wr-red)" /><strong>{routineDuration(draft)} min</strong><span>Estimado</span></div><div><Activity size={14} color="var(--wr-red)" /><strong>{muscleGroups.length}</strong><span>Grupos</span></div></div><button className="wr-btn wr-btn-primary" style={{ width: '100%' }} disabled={saving} onClick={save}><Save size={15} />{saving ? 'Guardando...' : 'Guardar rutina'}</button>{error && <div className="wr-validation" role="alert">{error}</div>}</aside>
      </div>
      {showPicker && <ExercisePicker catalog={catalog} selectedIds={new Set(items.map(item => item.exerciseId))} onAdd={addExercise} onClose={() => setShowPicker(false)} />}
    </>;
  };
  ...
  const HabitFlowWorkoutRedesign = ({ data, onUpdateData, onCompleteHabit, awardXp, onSync }) => {
    ...
    useEffect(() => {
      let cancelled = false;
      dataset.loadMeta().then(value => { if (!cancelled) setMeta(value); }).catch(() => {});
    ...
    let content;
    if (screen === 'summary') content = <SummaryView workoutData={workoutData} meta={meta} catalogById={catalogById} onNavigate={navigate} onEditRoutine={routine => openBuilder(routine)} onStartRoutine={startRoutine} />;
    ...
    return <div className={`wr-root ${screen === 'active' ? 'wr-session' : ''}`}>
      {screen !== 'detail' && <WorkoutHeader screen={screen} onNavigate={navigate} onNew={onNew} onSync={syncNow} syncing={syncing} />}
